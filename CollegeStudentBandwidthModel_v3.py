...
class CollegeStudentBandwidthModel:
    ...
    def plot_overlaid_devices(self, figsize=(16, 12), normalize=False):
        ...
        if normalize:
            ...
            for device, data in self.bandwidth_data.items():
                max_val = np.max(data)
                if max_val > 0:
                    plot_data[device] = data / max_val
                else:
                    plot_data[device] = data
            ylabel = 'Normalized Bandwidth (0-1 scale)'
            title_suffix = ' - Normalized View'
        else:
            plot_data = self.bandwidth_data
            ylabel = 'Bandwidth (Mbps)'
            title_suffix = ''
        
        # Plot each device as overlay
        for device in ['laptop', 'smartphone', 'tablet', 'smartwatch']:
            ...
            if device == 'smartwatch':
                ...
                ax_overlay.plot(self.hours, plot_data[device], color=color, 
                              linewidth=3, alpha=0.9, label=label, linestyle=style)
            else:
                ax_overlay.plot(self.hours, plot_data[device], color=color, 
                              linewidth=2.5, alpha=0.8, label=label, linestyle=style)
        
        ax_overlay.set_ylabel(ylabel, fontsize=12, fontweight='bold')
        ax_overlay.set_title(f'Device Bandwidth Overlay{title_suffix}', 
        ...
        for idx, device in enumerate(devices):
            ...
            if normalize:
                avg = np.mean(data)
                peak = np.max(data)
                ax.text(0.02, 0.98, f'Avg: {avg:.2f}\nPeak: {peak:.2f}',
                       transform=ax.transAxes, fontsize=8,
                       verticalalignment='top',
                       bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.7))
            else:
                avg = np.mean(self.bandwidth_data[device])
                peak = np.max(self.bandwidth_data[device])
...
if __name__ == "__main__":
    ...
    print("\n1. Creating standard bandwidth view...")
    fig1 = model.plot_overlaid_devices(normalize=False)
    
    print("2. Creating normalized view for relative comparison...")
    fig2 = model.plot_overlaid_devices(normalize=True)
    ...
    print("📊 Two views generated:")
    print("   - Absolute Mbps values")
    print("   - Normalized (0-1) for relative comparison")
